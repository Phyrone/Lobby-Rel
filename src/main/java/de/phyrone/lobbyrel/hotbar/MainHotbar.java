...
public class MainHotbar implements HotbarWrapper {
    static HashMap<Integer, HotbarItem> items = new HashMap<>();

    public static void setup() {
        items.clear();
        HotbarItem nav = new HotbarItem(new ItemBuilder(Material.COMPASS).displayname("&l&6Navigator").build());
        nav.setItemUpdater(player -> ItemsConfig.getInstance().getItem("Navigator", new ItemBuilder(Material.COMPASS).displayname("&l&6Navigator").build()).getAsItemStack(player));
        ...
        if (Config.getBoolean("Items.Navigator.Enabled", true))
            addItem(Config.getInt("Items.Navigator.Slot", 0), nav);
        if (Config.getBoolean("Items.PlayerHider.Enabled", true))
            addItem(Config.getInt("Items.PlayerHider.Slot", 1),
                    ...
                    }).setItemUpdater(player -> ItemsConfig.getInstance().getItem("PlayerHider",
        ...
        if (Config.getBoolean("Items.Settings.Enabled", true))
            addItem(Config.getInt("Items.Settings.Slot", 8),
                    ...
                    }).setItemUpdater(player -> ItemsConfig.getInstance().getItem("Settings",
        ...
        if (Config.getBoolean("Items.Switcher.Enabled", true))
            addItem(Config.getInt("Items.Switcher.Slot", 7), new HotbarItem(new ItemBuilder(Material.WATCH).displayname("&6Lobbys").build()).setClick((event, rightClick) -> {
                if (rightClick) LobbySwitcherGUI.open(event.getPlayer());
            }).setItemUpdater(player -> ItemsConfig.getInstance().getItem("LobbySwitcher", new ItemBuilder(Material.WATCH).displayname("&6Lobbys").build()).getAsItemStack(player)));
    }
