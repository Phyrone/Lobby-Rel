...
public class SettingsManager {
	...
	public static void setup() {
		...
		addModule( new SettingsModule(new ItemBuilder(Material.IRON_PLATE).displayname("&6JumpPad").build()).setOptions(
                new ArrayList<>(Arrays.asList(ClickableItem.of(new ItemBuilder(new ItemStack(Material.WOOL, 1, DyeColor.LIME.getDyeData())).displayname("&aJumpPad ON").build(), e -> {
					new OfflinePlayerData(e.getWhoClicked().getUniqueId()).setJumpPad(true);
                }), ClickableItem.of(new ItemBuilder(new ItemStack(Material.WOOL, 1, DyeColor.RED.getDyeData())).displayname("&cJumpPad OFF").build(), e -> {
