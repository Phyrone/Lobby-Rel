...
public class WarpManager {
    ...
    public static void saveToConf() {
        if (Bukkit.getPluginManager().isPluginEnabled(LobbyPlugin.getInstance())) {
            Bukkit.getScheduler().scheduleAsyncDelayedTask(LobbyPlugin.getInstance(), () -> {
                try {
    ...
    public static HashMap<String, Warp> getWarpsUnsorted() {
        return WarpConf.getInstance().Warps;
    }

    public static int getWarpSize() {
        return getWarpsUnsorted().size();
    }

    public static List<WarpPack> getWarps() {
        HashMap<String, Warp> unsortWarps = getWarpsUnsorted();
        List<WarpPack> warps = new ArrayList<>();
        for (String name : unsortWarps.keySet()) {
            warps.add(new WarpPack(name, unsortWarps.get(name)));
        }
        Collections.sort(warps, Comparator.comparingInt(o -> o.getWarp().getSort_ID()));
        return warps;
    }
    ...
    public static class WarpPack {
        private String name;
        private Warp warp;

        public WarpPack(String name, Warp warp) {
            this.name = name;
            this.warp = warp;
        }
